...
export async function setupVite(app: Express, server: Server) {
  ...
  // Correctly shape Vite's server options (no nested `server` inside `server`)
  const serverOptions = {
    middlewareMode: true,
    hmr: {
      server,
  ...
  // Attach HMR to the existing HTTP server so origin stays the same (avoids CSP issues)
  try {
    (vite.config.server as any).hmr = { ...(vite.config.server as any).hmr, server };
    const addrInfo: any = (server as any).address ? (server as any).address() : null;
    const boundPort = (addrInfo && addrInfo.port) ? addrInfo.port : process.env.PORT || 'unknown';
    console.log(`[vite-dev] HMR attached to parent HTTP server on port: ${boundPort}`);
  } catch (e) {
    // non-fatal
  }
